 ...
 * Corresponds to Python Step 2: Merging demographics and admission data
 */
public class DataMerger {
    
    /**
     * Merge ICUSTAYS with PATIENTS and ADMISSIONS
     * Step 2 from Python preprocessing
     */
    public Dataset<Row> mergeCoreData(Dataset<Row> icuStays, 
        ...
        Dataset<Row> df = icuStays.join(
            ...
            "left"
        ).drop(patientsSelected.col(Constants.COL_SUBJECT_ID)); // Drop duplicate SUBJECT_ID column
        ...
        df = df.join(
            ...
            "left"
        ).drop(admissionsSelected.col(Constants.COL_HADM_ID)); // Drop duplicate HADM_ID column
